Scholarship Core version 0.13.2 is now available from:
  ...
  <https://download.scholarshipcoin.org/scholarship-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a scholarship
...
ECDSA signatures inside Scholarship transactions now use validation using
...
This allows running scholarshipd without having to do any manual configuration.
...
Scholarshipd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Scholarship
...
`scholarship.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /scholarshiprpc>
        ...
        # optional bypass scholarshipd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< scholarshiprpc:<password>
    </Location>
...
Restarting scholarshipd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in scholarship.conf
...
scholarship-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/scholarship-cli -stdin walletpassphrase
    mysecretcode
    120
...
- `scholarship-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `scholarship-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Scholarship scripting system that in combination with
...
Scholarship transactions currently may specify a locktime indicating when
...
The two phases are distinct in the Scholarship-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Scholarship wallet
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Scholarship block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of scholarships they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of scholarships being spent is exactly
...
- **Increased security for multisig:** Scholarship addresses (both P2PKH addresses
  ...
  believed to be provided by Scholarship's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Scholarship's current ledger (called the UTXO set),
...
- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Scholarship users to individually opt-in to almost any change in the Scholarship
...
segwit's start date of 1 January 2017 miners can update the Scholarship
...
- `scholarship-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `scholarship-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
